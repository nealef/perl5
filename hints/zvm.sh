# hints/zvm.sh <-- keep the # character here
#
# z/VM hints by Neale Ferguson <neale@sinenomine.net>
#

# If we are cross-building
echo "Gathering z/VM Hints"
if [ `uname -s` != "z/VM" ]; then
    osname="zvm"
    case "$cc" in
    '') cc='xlc' ;;
    esac
...
def_zvm_cflags='-qlanglvl=extc1x';
...
def_zvm_defs="$def_zvm_defs -D_ALL_SOURCE";
...
def_zvm_cflags="$def_zvm_cflags -q32 -Wc,xplink -qascii"
def_zvm_cccdlflags="$def_zvm_cflags"
def_zvm_ldflags="-q32 -Wl,xplink /usr/local/lib/libascii_zos.x -qascii"
static_ldflags="-q32 -qxplink /usr/local/lib/libascii_zos.x -qascii"
libs=/usr/local/lib/libascii_zos.x -lm -lc
perllibs=$libs
...
def_zvm_cflags="$def_zvm_cflags -qexportall";
def_zvm_cccdlflags="$def_zvm_cccdlflags -qexportall"

archobjs="zvm.o"
ln -s zvm/zvm.c zvm.c
...
def_zvm_cflags="$def_zvm_cflags -qhaltonmsg=3296:4108 -qsuppress=CCN3159 -qfloat=ieee"

def_zvm_defs="$def_zvm_defs -DMAXSIG=39 -DNSIG=39";     # maximum signal number; not furnished by IBM
def_zvm_defs="$def_zvm_defs -DOEZVM";   # is used in place of #ifdef __MVS__

# ensure that the OS/390 yacc generated parser is reentrant.
def_zvm_defs="$def_zvm_defs -DYYDYNAMIC";

# LC_MESSAGES only affects the yes/no strings in langinfo; not the things we
# expect it to
def_zvm_defs="$def_zvm_defs -DNO_LOCALE_MESSAGES"

# Set up feature test macros required for features available on supported z/OS systems
def_zvm_defs="$def_zvm_defs -D_OPEN_THREADS=3 -D_UNIX03_SOURCE=1 -D_AE_BIMODAL=1 -D_ALL_SOURCE -D_ENHANCED_ASCII_EXT=0xFFFFFFFF -D_OPEN_SYS_FILE_EXT=1 -D_OPEN_SYS_SOCK_IPV6 -D_XOPEN_SOURCE=600 -D_XOPEN_SOURCE_EXTENDED -DNO_LOCALE_TIME"

# Combine -D with cflags
case "$ccflags" in
'') ccflags="$def_zvm_cflags $def_zvm_defs"  ;;
*)  ccflags="$ccflags $def_zvm_cflags $def_zvm_defs" ;;
esac
...
case "$cccdlflags" in
'') cccdlflags="$def_zvm_cccdlflags -Wl,dll";;
esac
...
define)
   ...
   cccdlflags="-c $def_zvm_cccdlflags"
   lddlflags="$def_zvm_cccdlflags"
   ...
   cat >config.arch <<'	EOCBU'
	...
	lddlflags="$lddlflags `pwd`/libperl.x /usr/local/lib/libascii_zos.x"
	EOCBU
   ;;
esac
